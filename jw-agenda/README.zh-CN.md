...
基于 [Agent Skills](https://agentskills.io/home) 的个人日程管理技能组。用自然语言驱动「📅 月规划 → 📊 周拆解 → ✅ 日执行 → 📝 日志记录 → 📈 周回顾」的完整闭环，在你真正干活的地方管理日程；所有数据以 Markdown 文件存储在本地。

**为什么用 jw-agenda？** 不用再切到日历或待办应用——留在编辑器里，和助手说句话，它会选对 skill 并同步你的工作区。
...
| 「同步规划」 | 🔍 检查全层级（tasks/年/月/周/日）一致性 → 📋 列出偏差 → ✨ 确认后修正 |
| 「周总结」 | 📊 汇总本周日志 → 📈 统计完成率和时间分配 → 📄 生成周报 |

### 核心特点

- **本地优先：** 数据全部在本地 Markdown 中，无云、完全由你掌控。
- **模块化：** 五个独立 skill 覆盖从规划到回顾的完整周期，可单独安装。
- **AI 原生：** 为「和 AI 对话」的工作方式设计，说人话即可，助手会选对 skill。
- **级联同步：** 规划类操作（daily-todo、weekly-plan）从上往下级联（tasks/TODO → 月 → 周 → 日）；回顾类操作（daily-log、weekly-review）从下往上轻量标记完成状态。各层自动保持一致。
...
事后全局一致性检查，覆盖 tasks/TODO、年规划、月规划、周规划、日计划全部层级。

**✨ 功能**：跨所有可用层级对比任务内容和完成状态；将偏差分为「需行动」和「信息性」两级；展示建议清单，仅在用户确认后才执行修改。作为其他 Skill 级联更新之后的全局验证，捕捉遗漏的偏差。

**💬 触发词**：「同步规划」「检查一致性」「planning sync」

**🔗 配合**：验证 daily-todo/weekly-plan 的从上往下级联和 daily-log/weekly-review 的从下往上状态同步是否完整。
...
| 📄 类型 | 📋 格式 | 📁 目录 | 💡 示例 |
|------|------|------|------|
| 📆 年规划（可选） | `YYYY-plan.md` | `yearly/` | `2026-plan.md` |
...
本项目是 **#BuildInPublic** 的一部分。若你使用 Cursor 或 Claude，且喜欢少干扰、基于 Markdown 的流程，欢迎反馈。贡献欢迎，详见 [CONTRIBUTING.md](CONTRIBUTING.md)。
