...
的目录之下），否则拒绝执行，避免路径遍历。调用方应传入 jw-agenda 根目录下的路径
（即已按用户配置或默认 jw-agenda-data 解析后的路径）。
...
def normalize(text: str) -> str:
    ...
    # 去掉来源标记：匹配 *(...)* 格式
    # 注意：[^)]+ 会在遇到第一个 ) 时停止，因此不支持嵌套括号
    # 例如 "*(从 (昨天) 转移)*" 只会匹配到 "*(从 (昨天)*"，留下 "转移)*"
    # 这是已知限制；当前来源标记（如 "*(从昨天转移)*"）均不含嵌套括号，故可接受
    text = re.sub(r"\*\([^)]+\)\*", "", text)
    # 去掉中文备注标记
    text = re.sub(r"（.*?）", "", text)
    # 去掉英文备注标记（与中文括号同等处理，避免英文条目去重失败）
    # 同样不支持嵌套括号，已知限制同上
    text = re.sub(r"\(.*?\)", "", text)
    # 去掉前后空格和标点
