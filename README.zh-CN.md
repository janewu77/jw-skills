...
**📋 用自然语言规划、执行和回顾你的个人日程。** jw-agenda 是一套完整的技能集，通过简单的对话就能自动化你的工作流程——从月度规划到每日执行再到周回顾。所有数据以 Markdown 文件形式本地存储，让你拥有完全的控制权和隐私保护。

**🔄 完整循环**：📅 月度规划 → 📊 周度拆解 → ✅ 每日执行 → 📝 日志记录 → 📈 周度回顾

只需简单地说：
- 💬 "生成本周计划" → 从月度目标创建按天拆解的计划
- 💬 "生成今天的计划" → 根据周计划和昨天未完成任务构建今日待办清单
- 💬 "我完成了简历更新" → 自动记录进度并更新复选框
- 💬 "把申请移到周三" → 重新安排任务并同步计划
- 💬 "周总结" → 汇总日志、计算完成率并识别待延续事项

所有 5 个技能无缝协作 🤝，但你也可以只安装需要的部分——每个技能都是独立的，能够优雅地处理缺失的依赖。
...
| 技能组 | 说明 |
|--------|------|
| [jw-agenda](jw-agenda/) | 📅 个人日程管理：月规划 → 周拆解 → 日执行 → 日志 → 周回顾。详见 [jw-agenda/README.md](jw-agenda/README.md)。 |
